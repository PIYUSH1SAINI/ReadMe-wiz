...
- For help, visit: https://github.com/PIYUSH1SAINI/ReadMe-wizard
  ...
  async function callGemini(promptText, apiKey = process.env.GEMINI_API_KEY) {
    try {
      ...
      if (
        error?.status === 400 &&
        error?.errorDetails?.some((e) => e.reason === "API_KEY_INVALID")
      ) {
        console.log(`
  ...
  Please double-check your key and try again.
        ...
        if (!retry.newKey) {
          console.log("❌ No valid API key entered. Exiting...");
          process.exit(1);
        }

        const newKey = retry.newKey.trim();

        // Write new key to .env in ReadMe Wizard folder
        fs.writeFileSync(envPath, `GEMINI_API_KEY=${newKey}\n`, "utf8");
        console.log("✅ .env file updated. Retrying Gemini API call...");

        // Retry call with new key passed explicitly
