...
// function generateMarkdownContributors(contributors) {
//   return contributors
//     .map((c) => `- [@${c.username}](https://github.com/${c.username})`)
//     .join("\n");
// }
...
function generateContributorsImages(contributors) {
  return (
    ...
    contributors
      .map(
        (c) => `<a href="https://github.com/${c.username}" target="_blank">
  <img src="https://github.com/${c.username}.png" width="60" height="60" style="border-radius: 50%; margin-right: 10px;" alt="@${c.username}" title="@${c.username}" />
</a>`
      )
      .join(" ") +
    "\n"
...
async function getProjectFiles() {
  const entries = await globby([
    "**/*.{js,ts,jsx,tsx,py,java,html,css,rb,php,go,cs,rs,cpp}",
    "!node_modules",
  ]);
  return entries;
}
...
function parseCode(filePath) {
  try {
    ...
    if ([".js", ".ts", ".jsx", ".tsx"].includes(ext)) {
      ...
      // Note: tree-sitter-vue is experimental; using JavaScript for JS parts
      parser.setLanguage(JavaScript);
    } else {
      return null;
    }
...
2. **Table of Contents**: Include a table of contents with links to all included sections (e.g., - [Description](#description)), placed immediately after the Title. Only list sections that are included in the final README.
...
- **Use Code Insights and Package.json**: Leverage code insights (e.g., Java methods, Python functions, Ruby methods, PHP functions, Go functions, C# methods, Rust functions, React/Vue components, HTML tags, CSS selectors) and package.json data to highlight unique aspects in Description, Features, Installation, Usage, and Scripts. Tailor commands to project type (e.g., npm for Node.js, pip for Python, mvn for Java, bundle for Ruby, composer for PHP, go for Go, dotnet for C#, cargo for Rust, none for HTML/CSS/Vue.js).
...
- **Omission Handling**: When omitting a section due to lack of meaningful data (e.g., empty scripts, insufficient files, or missing repository details), include a brief note in the README explaining the omission (e.g., "Scripts section omitted due to no meaningful scripts defined in package.json"). Ensure the table of contents only includes sections that are present in the final README.
...
async function generateReadme() {
  ...
  try {
    ...
    const contributors_markdown = generateContributorsImages(contributors);

    writeStream.write(contributors_markdown);
