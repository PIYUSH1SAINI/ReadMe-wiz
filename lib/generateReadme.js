...
async function generateReadme() {
  ...
  // 1. Check and ask for API key if missing
  if (!fs.existsSync(envPath) || !process.env.GEMINI_API_KEY) {
    ...
    console.log(
      `  2️⃣  Paste it below — we'll save it safely in a ${chalk.cyan(
        ".env"
      )} file.\n`
    );
    console.log(
      chalk.gray("💡 Need help?") +
        ` Visit: ${chalk.underline(
          "https://github.com/PIYUSH1SAINI/ReadMe-wizard"
        )}\n`
    );
  ...
  // 2. Validate .env still loaded
  if (!process.env.GEMINI_API_KEY) {
    console.log(
      error(`
❌ Failed to load the API key.
- Ensure your .env file exists in the project root.
- It should contain: GEMINI_API_KEY=your-api-key-here
`)
    );
    process.exit(1);
  }

  // 3. Call Gemini safely with retry logic
  async function callGemini(promptText, apiKey = process.env.GEMINI_API_KEY) {
    try {
      ...
      if (
        err?.status === 400 &&
        err?.errorDetails?.some((e) => e.reason === "API_KEY_INVALID")
      ) {
        ...
        const retry = await prompts({
          type: "text",
          name: "newKey",
          message: "🔁 Enter a valid Gemini API key to retry:",
          validate: (value) =>
            value.trim() ? true : "API key cannot be empty. Please try again.",
        });

        if (!retry.newKey) {
          console.log(error("❌ No valid API key entered. Exiting..."));
          process.exit(1);
        }

        const newKey = retry.newKey.trim();
        fs.writeFileSync(envPath, `GEMINI_API_KEY=${newKey}\n`, "utf8");

        console.log(
          success("✅ .env file updated. Retrying Gemini API call...")
        );
        return await callGemini(promptText, newKey);
      } else {
        console.error(error("❌ Unexpected error from Gemini API:"));
